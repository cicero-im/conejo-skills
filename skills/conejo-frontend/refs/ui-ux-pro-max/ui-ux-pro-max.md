---
name: ui-ux-pro-max
description: Heavyweight UI/UX reference (50+ styles, 161 palettes, 57 font pairings, 161 product types, 99 UX guidelines, 25 chart types, shadcn/ui MCP). LAST-RESORT skill — invoke ONLY when stuck on UX/UI decisions OR when the user explicitly expresses frustration with the UX (e.g. "this looks bad", "I hate this UI", "why is this so ugly", "fix the UX", "the design isn't working"). For normal design work prefer `impeccable` + design-dial skills (`distill`, `polish`, `delight`, etc.). Also includes the design-tokens / layout / motion / accessibility appendix from the former frontend-design-system.
---
...
## Appendix — Design Tokens & System Reference (from frontend-design-system)

When the user is frustrated or stuck, ground every recommendation in a token-driven, accessibility-checked system. This appendix gives you the canonical scaffolding.
...
export const tokens = {
  colors: {
    ...
    accent: '#F59E0B', success: '#10B981', warning: '#F59E0B', error: '#EF4444',
...
page_spec:
  ...
  hierarchy:
    ...
    secondary_actions: [<secondary CTAs>]
    information_architecture:
      - { section: hero,         priority: 1 }
      - { section: features,     priority: 2 }
      - { section: social_proof, priority: 3 }
      - { section: cta,          priority: 4 }
  responsive:
    mobile_first: true
...
- [ ] Text/background contrast ≥ 4.5:1 (normal text), ≥ 3:1 (large text), ≥ 3:1 (UI components)
- [ ] All interactive elements focusable, focus order logical, focus indicator visible, skip links present
- [ ] Semantic HTML, images have alt text, form labels associated, ARIA labels where needed
- [ ] Body text ≥ 16px, line-height ≥ 1.5, paragraph width < 80 chars, no text-in-images

### Token-Driven Handoff Template

```
## Tokens Applied:    colors, typography, spacing, shadows
## Section Layout:    grid + components per section
## Component List:    button (primary/secondary/ghost), card (elevated/flat), input (default/error), badge, icon
## Accessibility:     contrast / focus / semantic-html / keyboard-nav
```

### Common Pitfalls When Rescuing a Broken UX

1. **Generic feel** — no tokens defined. Fix: lock in palette + type scale + spacing scale first.
2. **Breaks on mobile** — no responsive rules. Fix: define breakpoints + stacking behavior.
3. **Inconsistent components** — tokens not referenced. Fix: route all values through `tokens.*`.
4. **Over-animation / over-gradient** — pick fewer, sharper effects.
5. **Wrong hierarchy** — UI follows content priority; rebuild from the primary action outward.
